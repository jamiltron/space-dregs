...
/** A slim white dart riding the bullet contact path (damage 4), with a
 *  lifetime for the motor burn. Guidance runs in system_missiles. */
Entity missile_spawn(World *world, Vec2f position, float angle) {
  ...
  world->bullets[e] = (Bullet){ .hostile = false, .damage = MISSILE_DAMAGE };
